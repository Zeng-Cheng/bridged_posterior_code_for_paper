...
gibbs_sampler <- function(
    laps, Zres, list_lambdas, dist_mat, num_iterations
) {
    S <- length(laps)
    ...
    loss_all = sapply(1:S, function(s) {
        ...
        for (j in 1:Klam) {
            LAs = laps[[s]]
            list_dist = c(list_dist, sum((LAs - Zres[[s]][[j]]) ^ 2))
        }
        list_dist
    })
    ...
    alpha_gamma <- 2
    beta_gamma <- var(dist_mean) # inv-gamma prior for gamma

    # inv-gamma prior for sigma2
    alpha_sigma <- 2
    beta_sigma <- var(c(colMeans(sqrt(loss_all))))
    ...
    updateLambda <- function(lambdas) {
        for (s in 1:S){
            ...
            for (j in 1:Klam){
                ...
                loglik[j] <- loglik[j] - sum(all_dist) / (2 * (S - 1) * gamma)
            }
    ...
    updateGamma <- function(lambdas) {
        ...
        rate <- beta_gamma + sum_dist / 2 / (2 * (S - 1))
    ...
    updateSigma2 <- function(lambdas) {
        shape <- alpha_sigma + S * 3 / 2
        all_losses <- sapply(1:S, function(s) {
            loss_all[lambdas[s], s]
        })
        all_norms <- sapply(1:S, function(s) {
            list_lambdas[[s]][lambdas[s]] * star_norm_all[lambdas[s], s]
        })
    ...
    # initial values
    lambdas <- sapply(1:S, function(s) {
        sample(1:Klam, size=1, replace=TRUE)})
